...
var palindrome = function(string) {
  let space = string.replace(" ", "").toUpperCase();
  
  //base
    if (space.length === 1){
      return true;
    } //recursion
    else if (space.length === 2 && space.charAt(0) === space[space.length - 1]){
    return true;
  }
    else if (space.charAt(0) !== space[space.length - 1]){
    return false;
    }
    return palindrome(space.slice(1, space.length - 1))
};
...
var nthFibo = function(n, fib = [0, 1]) {
  ...
  let newValue = fib[fib.length - 1] + fib[fib.length - 2];
...
var capitalizeFirst = function(array, arr = []) {
  ...
  arr.push(array[0].charAt(0).toUpperCase() + array[0].slice(1).toLowerCase());
...
var minimizeZeroes = function(array, arr = []) {
  ...
  if (arr[arr.length - 1] === 0 && array[0] === 0){
...
var numToText = function(str, arr = []) {
str.split(" ");
  if (str.length === 0){
    return arr.join("");
  }
  ...
  return numToText(str.slice(1), arr)
};
