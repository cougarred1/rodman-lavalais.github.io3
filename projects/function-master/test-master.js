(function(){
  ...
  QUnit.test( "Our First Test", function( assert ) { //test case
    var value = "hello tests";
    var some_number = 484;

    //                     ┌ Change this to what it should be
    assert.equal( value, 'hello tests'); //'hello tests' make sure these pass
    //                           ┌ Change this to what it should be
    assert.equal( some_number, 484); //484
  });
  ...
  QUnit.test("Functions can access/modify variables in parent scope.", function(assert){
    ...
    function yay(){
      ...
      outside_the_function = inside_the_function; //outside the function is pointing ast inside
    }
